(() => {
  ...
  var login = async (email, password) => {
    try {
      ...
      if (res.data.status === "success") {
        showAlert("success", "Logged in succesfullly!");
        window.setTimeout(() => {
          location.assign("/");
        }, 1500);
      }
  ...
  var displayMap = (locations) => {
    mapboxgl.accessToken = "pk.eyJ1IjoiZHV0aG5nIiwiYSI6ImNseDVwOXhvdjA3M3gyanB5eTZ4ZXN6aGoifQ.YKRpiQtaCCQb-ya1tc9kDg";
    const map = new mapboxgl.Map({
      container: "map",
      // container ID
      style: "mapbox://styles/duthng/clx5q4ypv03ci01qm6l8ngd7r",
      // style URL
      center: [-118.11, 34.111745],
